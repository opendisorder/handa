...
> **Date:** 2026-06-10 (Updated with Memory & Psychology Extensions)
> **Status:** COMPLETE
...
## Sprint 6: Gemini Live Integration via Vertex AI Bidi (M3 — Days 22-27)

**Goal:** Gemini Live WebSocket connection via Vertex AI Bidi streaming endpoint with ADC authentication.

> **Note:** We removed the `gemini_live` package dependency (requires Dart SDK ≥3.8.1; we have 3.7.2). Instead, we implement a custom WebSocket client connecting to Vertex AI's BidiGenerateContent endpoint using OAuth2 tokens from ADC (Application Default Credentials). We have working Vertex AI access verified via `biz-studio-1779528000` project.

| Order | File | Description | Depends On |
|-------|------|-------------|------------|
| 6.1 | `lib/core/network/vertex_ai_auth.dart` | Manages ADC OAuth2 token acquisition & refresh | — |
| 6.2 | `lib/data/datasources/remote/gemini_live_client.dart` | Custom Vertex AI Bidi WebSocket client | 6.1 |
| 6.3 | `lib/data/datasources/remote/cloudflare_proxy_source.dart` | Cloudflare Worker for optional server-side auth | — |
| 6.4 | **`lib/domain/usecases/sync_data.dart`** | Cloud sync orchestrator | — |
| 6.5 | `lib/presentation/providers/live_session_provider.dart` | Live conversation state (Riverpod) | 6.2 |
| 6.6 | `lib/presentation/pages/session/live_conversation/live_conversation_page.dart` | Basic live conversation UI | 2.7, 2.10, 6.5 |
| 6.7 | `lib/presentation/pages/session/live_conversation/live_summary_page.dart` | Live session results | 2.10, 4.2 |
| 6.8 | `workers/gemini-proxy/src/index.ts` | Cloudflare Worker for token relay | — |
| 6.9 | `workers/gemini-proxy/wrangler.toml` | Worker configuration | 6.8 |

**Gemini Live Vertex AI Bidi Protocol:**
```
Client (Flutter) ←WebSocket→ Vertex AI BidiGenerateContent
  Auth: OAuth2 Bearer token from gcloud ADC
  URL: wss://us-central1-aiplatform.googleapis.com/ws/...
  Model: gemini-2.5-flash
  Send: BidiGenerateContentClientMessage (audio chunks + text)
  Receive: BidiGenerateContentServerMessage (text + audio)
```

**M3 Validation Gate:**
- WebSocket connection establishes with Vertex AI Bidi endpoint
- OAuth2 token acquisition from ADC works and refreshes
- User speaks → AI hears → AI responds with audio
- Live UI shows waveform, transcript, timer
- Cloudflare Worker deployed for token relay

---

## Sprint 7: AI Persona & Therapeutic Protocol (M3a — Days 28-32)

**Goal:** Implement "Thilina" AI persona and the 6-phase session structure.

| Order | File | Description | Depends On |
|-------|------|-------------|------------|
| 7.1 | `lib/core/ai/ai_persona_config.dart` | "Thilina" persona config: name, voice, behavioral rules | 6.1 |
| 7.2 | `lib/core/ai/session_phase_controller.dart` | Tracks which of 6 phases we're in, manages transitions | — |
| 7.3 | `lib/core/ai/cueing_ladder.dart` | 5-level cueing ladder logic (0-5) with timer | — |
| 7.4 | `lib/core/ai/silence_detector.dart` | Client-side silence timer for cueing triggers | 3.4 |
| 7.5 | `lib/core/ai/struggle_classifier.dart` | Combines silence + facial signals → struggle level | 7.3, 7.4 |
| 7.6 | `lib/core/ai/system_prompt_builder.dart` | Builds Gemini Live system prompt with memory + persona | — |
| 7.7 | `lib/core/ai/psychological_techniques.dart` | Identity reinforcement, validation, containment phrases | — |
| 7.8 | `lib/data/datasources/platform/tts_source_sinhala.dart` | Piper TTS for Sinhala (for AI voice + feedback) | 3.6 |
| 7.9 | Updated system prompt file | `assets/prompts/thilina_persona_v1.txt` | 7.1, 7.6 |

**Session Phase Structure (implemented in 7.2):**
```
Phase 1: Warm-Up (3 min) — breathing + easy wins
Phase 2: Naming (8 min) — cueing ladder active
Phase 3: Repetition (5 min) — syllable/word drills
Phase 4: Fluency (5 min) — pacing exercises
Phase 5: Functional (5 min) — conversation/scripts
Phase 6: Cool-Down (3 min) — breathing + containment
```

**M3a Validation Gate:**
- AI introduces itself as "Thilina, speech therapist"
- Cueing ladder triggers at correct intervals
- 6-phase session structure flows correctly
- Breathing integrates at all phase points
- AI maintains persona consistently across 3 test conversations

---

## Sprint 8: Memory & Background Agent System (M4 — Days 33-39)

**Goal:** Two-layer memory architecture — background agent for post-session analysis + memory preload for next session.

| Order | File | Description | Depends On |
|-------|------|-------------|------------|
| 8.1 | `lib/data/datasources/remote/firestore_source.dart` | Firestore CRUD operations | — |
| 8.2 | `lib/data/memory/memory_models.dart` | SessionMemory, PsychologicalMarkers models | — |
| 8.3 | `lib/data/memory/memory_store.dart` | Firestore CRUD for session memories | 8.1, 8.2 |
| 8.4 | `lib/data/memory/prompt_builder.dart` | Builds system prompt with memory preload | 8.3 |
| 8.5 | `lib/data/background_agent/agent_prompt_builder.dart` | Builds Gemini Flash prompt for session analysis | — |
| 8.6 | `lib/data/background_agent/agent_response_parser.dart` | Parses Gemini response JSON | — |
| 8.7 | `lib/data/background_agent/agent_orchestrator.dart` | Orchestrates: upload → call agent → parse → save → report | 8.5, 8.6, 8.1 |
| 8.8 | `lib/data/background_agent/report_generator.dart` | Generates caregiver weekly report | 8.7 |
| 8.9 | `lib/data/datasources/platform/audio_recorder.dart` | Full session audio recorder (uploads after session) | 3.4 |
| 8.10 | `lib/data/datasources/platform/key_frame_sampler.dart` | Samples 10-20 video frames during session | — |
| 8.11 | `lib/presentation/providers/memory_provider.dart` | Memory provider for UI | 8.3 |
| 8.12 | `lib/presentation/providers/report_provider.dart` | Report provider for caregiver | 8.8 |

**M4 Validation Gate:**
- Session recording captures full audio + frames
- Background agent processes session and returns valid JSON
- Memory preload injected into next session system prompt
- Caregiver weekly report generated correctly

---

## Sprint 9: Psychological Features & Safety (M5 — Days 40-44)

**Goal:** Red flag detection, caregiver escalation, psychological dashboard.

| Order | File | Description | Depends On |
|-------|------|-------------|------------|
| 9.1 | `lib/core/ai/red_flag_detector.dart` | Keyword + context detection for psychological red flags | — |
| 9.2 | `lib/core/ai/escalation_handler.dart` | Push notification + Firestore log on red flag | 9.1 |
| 9.3 | `lib/data/datasources/remote/notification_source.dart` | Firebase Cloud Messaging wrapper | — |
| 9.4 | `lib/domain/entities/psychological_marker.dart` | Psychological marker entity | — |
| 9.5 | `lib/presentation/providers/psychology_provider.dart` | Psychology data provider | 8.7 |
| 9.6 | `lib/presentation/pages/caregiver/psychological/mood_trend_chart.dart` | Mood trend line chart (fl_chart) | 9.5 |
| 9.7 | `lib/presentation/pages/caregiver/psychological/negative_statements_chart.dart` | Negative statements over time | 9.5 |
| 9.8 | `lib/presentation/pages/caregiver/psychological/red_flag_log.dart` | Red flag incident log | 9.2 |
| 9.9 | `lib/presentation/pages/caregiver/psychological/weekly_insights.dart` | AI-generated weekly insight report | 8.8, 9.5 |

**M5 Validation Gate:**
- Red flag keyword detection works (test with sample phrases)
- Push notification sent to caregiver on red flag
- Psychological charts render with correct data
- Weekly insight report combines speech + psychology

---

## Sprint 10: Caregiver Dashboard — Extended (M6 — Days 45-49)

**Goal:** Full caregiver dashboard with psychology tab, PDF export with psychological data.

| Order | File | Description | Depends On |
|-------|------|-------------|------------|
| 10.1 | `lib/presentation/pages/caregiver/pin_entry/pin_entry_page.dart` | PIN screen with number pad | 2.10, 2.11 |
| 10.2 | `lib/presentation/pages/caregiver/dashboard/dashboard_page.dart` | Dashboard home with speech + psychology stats | 2.8, 2.10, 1.19 |
| 10.3 | `lib/presentation/pages/caregiver/sessions/session_history_page.dart` | Session list with filters | 2.10, 4.15 |
| 10.4 | `lib/presentation/pages/caregiver/sessions/session_detail_page.dart` | Session detail with replay | 2.10, 4.4 |
| 10.5 | `lib/presentation/pages/caregiver/analytics/analytics_page.dart` | Charts page container | 2.10 |
| 10.6 | `lib/presentation/pages/caregiver/analytics/score_trend_chart.dart` | Speech accuracy line chart | 1.19 |
| 10.7 | `lib/presentation/pages/caregiver/analytics/cueing_trend_chart.dart` | Cueing level trend over time | 7.3 |
| 10.8 | `lib/presentation/pages/caregiver/content/content_browser_page.dart` | Image categories browser | 2.10 |
| 10.9 | `lib/presentation/pages/caregiver/content/add_photo_page.dart` | Custom photo upload | 2.10 |
| 10.10 | `lib/presentation/pages/caregiver/settings/settings_page.dart` | Settings panel | 2.10, 1.20 |
| 10.11 | `lib/presentation/pages/caregiver/export/pdf_export_page.dart` | PDF export with speech + psychology | 8.8 |
| 10.12 | `lib/presentation/pages/patient_settings/patient_settings_page.dart` | Basic patient settings | 2.10 |

**M6 Validation Gate:**
- PIN protects caregiver access (5-attempt lockout)
- Dashboard shows both speech + psychology data
- Psychological charts render correctly
- PDF export includes psychology section
- Settings persist across app restarts

---

## Sprint 11: Cloud Sync + On-Device Detection (M7a — Days 50-54)

**Goal:** Data sync, optional MediaPipe face detection, multi-language.

| Order | File | Description | Depends On |
|-------|------|-------------|------------|
| 11.1 | `lib/data/datasources/remote/firestore_source.dart` | Firestore CRUD operations | — |
| 11.2 | `lib/data/repositories/sync_repository_impl.dart` | Sync queue implementation | 11.1 |
| 11.3 | `lib/presentation/providers/sync_provider.dart` | Sync status provider | 11.2 |
| 11.4 | `lib/data/datasources/detectors/face_mesh_detector.dart` | MediaPipe Face Mesh (optional, silence-only fallback) | — |
| 11.5 | `lib/data/datasources/detectors/motion_detector.dart` | Head turning detection | — |
| 11.6 | `lib/l10n/app_si.arb` | Sinhala localization strings | — |
| 11.7 | `lib/l10n/app_ta.arb` | Tamil localization strings | — |
| 11.8 | `lib/l10n/app_en.arb` | English localization strings | — |
| 11.9 | `lib/domain/usecases/manage_language_unlock.dart` | Language progression logic | — |

---

## Sprint 12: Testing, Polish & Deploy (M7b — Days 55-60)

**Goal:** Comprehensive testing, optimization, and APK release.

| Order | Task | Description |
|-------|------|-------------|
| 12.1 | Unit tests — Scoring Engine | ≥95% coverage |
| 12.2 | Unit tests — Cueing Ladder | Test all 5 levels trigger correctly |
| 12.3 | Unit tests — Red Flag Detector | Test keyword detection accuracy |
| 12.4 | Unit tests — Background Agent Parser | Test JSON parsing edge cases |
| 12.5 | Unit tests — Memory Store | CRUD + edge cases |
| 12.6 | Integration test — Session Flow | Full 6-phase session end-to-end |
| 12.7 | Integration test — Caregiver Flow | PIN → dashboard → export |
| 12.8 | Persona consistency test | 3 test conversations, verify persona rules |
| 12.9 | Performance audit | DevTools: verify 55+ fps, <16ms frames |
| 12.10 | Accessibility audit | TalkBack navigation, contrast check |
| 12.11 | Error hardening | Force errors → verify graceful handling |
| 12.12 | Sinhala content review | Native speaker verifies all strings |
| 12.13 | App icon + splash | Branded assets |
| 12.14 | APK build | `flutter build apk --release`, sign |
| 12.15 | Install test | On target device (father's tablet) |
